...
const PrismaticBurst = ({
  ...
  useEffect(() => {
    ...
    try {
      ...
      gl = renderer.gl;
      if (!gl || !gl.canvas) throw new Error("No WebGL context");
    } catch (err) {
      console.warn("PrismaticBurst WebGL creation failed:", err);
      return;
    }
    ...
    return () => {
      ...
      try {
        if (typeof meshRef.current?.remove === 'function') meshRef.current.remove();
        if (typeof meshRef.current?.setParent === 'function') meshRef.current.setParent(null);
      } catch (e) {
        /* ignore dispose errors */
      }
      try {
        if (typeof triRef.current?.remove === 'function') triRef.current.remove();
      } catch (e) {
        /* ignore dispose errors */
      }
      try {
        if (typeof programRef.current?.remove === 'function') programRef.current.remove();
      } catch (e) {
        /* ignore dispose errors */
      }
      try {
        if (typeof gradTexRef.current?.dispose === 'function') {
          gradTexRef.current.dispose();
        }
      } catch (e) {
        /* ignore texture dispose errors */
      }
