...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
  ...
  if (jitterAmt > 0.0) {
    vec2 j = vec2(
      sin(gridUV.y * 1.2 + iTime * 0.4),
      cos(gridUV.x * 1.1 - iTime * 0.3)
    ) * (0.02 * jitterAmt);
    gridUV += j;
  }
  ...
  if (jitterAmt > 0.0) {
    vec2 j2 = vec2(
      cos(gridUV2.y * 1.1 - iTime * 0.35),
      sin(gridUV2.x * 1.2 + iTime * 0.4)
    ) * (0.02 * jitterAmt);
    gridUV2 += j2;
  }
...
export const GridScan = ({
  ...
  const paramsRef = useRef({ smoothTime, maxSpeed, skewScale, yBoost, tiltScale, yawScale });
  paramsRef.current = { smoothTime, maxSpeed, skewScale, yBoost, tiltScale, yawScale };
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.warn("WebGL creation failed in GridScan:", err);
      return;
    }
    ...
    const getDimensions = () => ({
      w: Math.max(1, Math.min(container.clientWidth || window.innerWidth || 1920, 4096)),
      h: Math.max(1, Math.min(container.clientHeight || window.innerHeight || 1080, 4096))
    });

    const initSize = getDimensions();
    ...
    const uniforms = {
      iResolution: {
        value: new THREE.Vector3(initSize.w, initSize.h, renderer.getPixelRatio())
      },
      iTime: { value: 0 },
    ...
    const tick = () => {
      ...
      const { smoothTime: st, maxSpeed: ms, skewScale: ss, yBoost: yb, tiltScale: ts, yawScale: ys } = paramsRef.current;

      lookCurrent.current.copy(
        smoothDampVec2(lookCurrent.current, lookTarget.current, lookVel.current, st, ms, dt)
      );
      ...
      const skew = new THREE.Vector2(lookCurrent.current.x * ss, -lookCurrent.current.y * yb * ss);
      material.uniforms.uSkew.value.set(skew.x, skew.y);
      material.uniforms.uTilt.value = tiltCurrent.current * ts;
      material.uniforms.uYaw.value = THREE.MathUtils.clamp(yawCurrent.current * ys, -0.6, 0.6);
    ...
    // Render immediate initial frame to prevent blank canvas flash on mount
    material.uniforms.iTime.value = last / 1000;
    renderer.clear(true, true, true);
    if (composerRef.current) {
      composerRef.current.render(0.016);
    } else {
      renderer.render(scene, camera);
    }
